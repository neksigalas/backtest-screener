"""
Hourly price monitor for New Tactic 3 Best Stocks Paper Trading.
Same exit logic as original: TP +10% / SL -10%.
"""
...
def run_monitor():
    ...
    if not state["positions"]:
        print("No open NT positions to monitor.")
        return False

    print(f"[New Tactic] Monitoring {len(state['positions'])} open position(s)...")
    changed = False
    ...
    for pos in list(state["positions"]):
        ...
        print(f"  {ticker}: entry=${entry_price:.2f}  now=${price:.2f}  P&L={pnl_pct:+.1f}%")
