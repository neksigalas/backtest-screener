"""
Daily report for New Tactic 3 Best Stocks Paper Trading.
Same visual style as original but teal/green colour scheme to distinguish it.
Shows extra columns: MACD status, reversal signal, market regime.
"""
...
from new_tactic_trader.engine import load_state, portfolio_stats, TAKE_PROFIT_PCT, STOP_LOSS_PCT

TITLE = "New Tactic 3 Best Stocks"
...
# ── Signal badges shown in entry rows ─────────────────────────────────────────

def _signal_tags(pos: dict) -> str:
    tags = []
    if pos.get("entry_reversal"):
        tags.append('<span style="background:#064e3b;color:#6ee7b7;padding:1px 6px;border-radius:6px;font-size:10px;">↩ REVERSAL</span>')
    if pos.get("entry_macd"):
        tags.append('<span style="background:#1e3a5f;color:#93c5fd;padding:1px 6px;border-radius:6px;font-size:10px;">⬆ MACD</span>')
    if pos.get("entry_regime") is False:
        tags.append('<span style="background:#450a0a;color:#fca5a5;padding:1px 6px;border-radius:6px;font-size:10px;">🐻 BEAR</span>')
...
def build_report(state: dict, today_str: str) -> str:
    ...
    for p in today_buys:
        ...
        <tr style="border-bottom:1px solid #0f172a;">
          <td style="padding:10px 8px;">
            <span style="background:#064e3b;color:#6ee7b7;padding:2px 8px;border-radius:10px;font-size:11px;">🛒 BUY</span>
          </td>
          <td style="padding:10px 8px;font-weight:700;color:#f1f5f9;">{p['ticker']}</td>
          <td style="padding:10px 8px;color:#94a3b8;">${p['entry_price']:.2f}</td>
          <td style="padding:10px 8px;">{_signal_tags(p)}</td>
          <td style="padding:10px 8px;">
    ...
    for t in today_trades:
        ...
        <tr style="border-bottom:1px solid #0f172a;">
          ...
          <td style="padding:10px 8px;font-weight:700;color:#f1f5f9;">{t['ticker']}</td>
    ...
    if not activity_rows:
        activity_rows = """
        <tr><td colspan="6" style="padding:20px;text-align:center;color:#475569;">
          Καμία κίνηση σήμερα
        </td></tr>"""
    ...
    activity_headers = """
        <tr style="background:#0f172a;">
          ...
          <th style="padding:10px 8px;text-align:left;color:#64748b;font-size:11px;">ΜΕΤΟΧΗ</th>
          <th style="padding:10px 8px;text-align:left;color:#64748b;font-size:11px;">ΤΙΜΗ ΑΓΟΡΑΣ</th>
          <th style="padding:10px 8px;text-align:left;color:#64748b;font-size:11px;">SIGNALS</th>
          <th style="padding:10px 8px;text-align:left;color:#64748b;font-size:11px;">SCORE / P&L</th>
        </tr>"""
    ...
    for p in pos_with_price:
        ...
        <tr style="border-bottom:1px solid #0f172a;">
          ...
          <td style="padding:10px 8px;">{_signal_tags(p)}</td>
          <td style="padding:10px 8px;">
            <span style="background:#064e3b;color:#6ee7b7;padding:2px 6px;border-radius:8px;font-size:11px;">{p['entry_score']}</span>
          </td>
        </tr>"""
...
<div style="max-width:900px;margin:0 auto;padding:24px 16px;">
  ...
  <!-- Header -->
  <div style="background:linear-gradient(135deg,#022c22,#0f172a);border-radius:14px;padding:28px;margin-bottom:24px;border:1px solid #10b98122;">
    <div style="font-size:11px;color:#34d399;font-weight:600;letter-spacing:.1em;text-transform:uppercase;margin-bottom:6px;">Daily Report — Improved Entry Strategy</div>
    ...
    <div style="margin-top:12px;display:flex;gap:12px;flex-wrap:wrap;font-size:12px;">
      <span style="background:#064e3b;color:#6ee7b7;padding:3px 10px;border-radius:8px;">↩ Reversal Confirmation</span>
      <span style="background:#1e3a5f;color:#93c5fd;padding:3px 10px;border-radius:8px;">⬆ MACD Cross</span>
      <span style="background:#1a1040;color:#c4b5fd;padding:3px 10px;border-radius:8px;">📈 Market Regime (SPY 50d MA)</span>
    </div>
  </div>
  ...
  <div style="padding:16px;background:#1e293b;border-radius:10px;font-size:12px;color:#475569;line-height:1.8;">
    ...
    <div>🆕 Νέα σήματα: ↩ Reversal confirmation + ⬆ MACD cross + 📈 SPY regime filter</div>
    <div>📡 Data: Yahoo Finance | Scan: 22:00 Αθήνα (Δε-Πα) | Monitor: κάθε ώρα</div>
  </div>
...
def send_report():
    ...
    print("Building New Tactic report...")
    ...
    print(f"Sending New Tactic report to {to_addr}...")
    with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
        server.login(gmail_user, gmail_pass)
        server.sendmail(gmail_user, to_addr, msg.as_string())
    print("New Tactic report sent!")
