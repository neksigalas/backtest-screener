"""
Evening scan for 3 Best Crypto Paper Trading.
Runs after NYSE close: scores all top cryptos, opens paper positions for
candidates with score >= SCORE_THRESHOLD (65), up to MAX_POSITIONS (3) total.
Crypto trades 24/7, so the scan still runs at 22:00 Athens for consistency.
"""
...
from crypto_trader.screener import get_candidates
...
def run_scan() -> list[dict]:
    ...
    if not candidates:
        print(f"No cryptos found with score >= {SCORE_THRESHOLD}.")
        save_state(state)
        return []
    ...
    for s in candidates[:5]:
        print(
            f"  {s['ticker']:12s} {s['name'][:20]:20s}  "
            f"score={s['score']}  RSI={s['rsi']}  "
            f"rank=#{s.get('market_cap_rank', '?')}  7d={s.get('change_7d', '?')}%"
        )
