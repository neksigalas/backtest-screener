"""
Daily report builder and email sender for 3 Best Crypto Paper Trading.
...
from crypto_trader.engine import load_state, portfolio_stats, TAKE_PROFIT_PCT, STOP_LOSS_PCT

TITLE = "3 Best Crypto Paper Trading"
...
def build_report(state: dict, today_str: str) -> str:
    ...
    for p in today_buys:
        ...
        <tr style="border-bottom:1px solid #0f172a;">
          ...
          <td style="padding:10px 8px;color:#94a3b8;">{_fmt_price(p['entry_price'])}</td>
    ...
    for t in today_trades:
        ...
        <tr style="border-bottom:1px solid #0f172a;">
          ...
          <td style="padding:10px 8px;color:#94a3b8;">{_fmt_price(t['entry_price'])}</td>
          <td style="padding:10px 8px;color:#94a3b8;">{_fmt_price(t['exit_price'])}</td>
    ...
    for p in pos_with_price:
        ...
        tp_price = round(p["entry_price"] * (1 + abs(TAKE_PROFIT_PCT)), 8)
        sl_price = round(p["entry_price"] * (1 - abs(STOP_LOSS_PCT)), 8)
        ...
        <tr style="border-bottom:1px solid #0f172a;">
          ...
          <td style="padding:10px 8px;color:#94a3b8;">{_fmt_price(p['entry_price'])}</td>
          <td style="padding:10px 8px;color:#f1f5f9;font-weight:600;">{_fmt_price(p['current_price'])}</td>
          ...
          <td style="padding:10px 8px;color:#22c55e;font-size:12px;">{_fmt_price(tp_price)}</td>
          <td style="padding:10px 8px;color:#ef4444;font-size:12px;">{_fmt_price(sl_price)}</td>
          <td style="padding:10px 8px;">
...
<div style="max-width:900px;margin:0 auto;padding:24px 16px;">
  ...
  <!-- Header -->
  <div style="background:linear-gradient(135deg,#1a1040,#0f172a);border-radius:14px;padding:28px;margin-bottom:24px;border:1px solid #7c3aed22;">
    <div style="font-size:11px;color:#a78bfa;font-weight:600;letter-spacing:.1em;text-transform:uppercase;margin-bottom:6px;">Daily Report</div>
  ...
  <div style="padding:16px;background:#1e293b;border-radius:10px;font-size:12px;color:#475569;line-height:1.8;">
    ...
    <div>🎯 Κανόνες: Max 3 θέσεις | $100/θέση | Take Profit +10% | Stop Loss -10% | Score &gt; 65</div>
    <div>📡 Data: CoinGecko + Yahoo Finance | Scan: καθημερινά 22:00 | Monitor: κάθε 2 ώρες 24/7</div>
  </div>
...
def send_report():
    ...
    print(f"Sending crypto report to {to_addr}...")
