"""
State management and core trading logic for 3 Best Crypto Paper Trading.
Mirrors paper_trader/engine.py — same rules, separate state file.
"""
...
SCORE_THRESHOLD  = 65     # slightly lower: crypto is more volatile
...
def open_position(state: dict, ticker: str, name: str, price: float, score: int) -> bool:
    ...
    quantity = round(COST_PER_TRADE / price, 8)
    ...
    print(f"  BUY  {ticker} ({name}) @ ${price:.4f}  qty={quantity:.6f}  score={score}")
    return True
...
def close_position(state: dict, ticker: str, exit_price: float, reason: str) -> dict | None:
    ...
    emoji = "✅" if pnl >= 0 else "❌"
    print(f"  SELL {ticker} @ ${exit_price:.4f}  P&L={pnl_pct:+.1f}%  [{reason}] {emoji}")
    return trade
