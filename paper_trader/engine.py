"""
State management and core trading logic for 3 Best Stocks Paper Trading.

Handles reading/writing state.json, opening and closing paper positions.
"""
...
COST_PER_TRADE   = 100.0   # $ invested per position
MAX_POSITIONS    = 3
...
def open_position(state: dict, ticker: str, price: float, score: int) -> bool:
    """Paper-buy a stock. Returns True if position was opened."""
    ...
    print(f"  BUY  {ticker} @ ${price:.2f}  qty={quantity:.4f}  score={score}")
    return True


def close_position(state: dict, ticker: str, exit_price: float, reason: str) -> dict | None:
    """Paper-sell a stock and record the completed trade. Returns the trade dict."""
