"""
Daily report builder and email sender for 3 Best Stocks Paper Trading.
...
def build_report(state: dict, today_str: str) -> str:
    ...
    if not pos_rows:
        pos_rows = """
        <tr><td colspan="9" style="padding:20px;text-align:center;color:#475569;">
...
<div style="max-width:860px;margin:0 auto;padding:24px 16px;">
  ...
  <!-- Header -->
  <div style="background:linear-gradient(135deg,#1e3a5f,#0f172a);border-radius:14px;padding:28px;margin-bottom:24px;">
    <div style="font-size:11px;color:#60a5fa;font-weight:600;letter-spacing:.1em;text-transform:uppercase;margin-bottom:6px;">Daily Report</div>
  ...
  <div style="padding:16px;background:#1e293b;border-radius:10px;font-size:12px;color:#475569;line-height:1.8;">
    ...
    <div>🎯 Κανόνες: Max 3 θέσεις | $100/θέση | Take Profit +10% | Stop Loss -10% | Score &gt; 75</div>
    <div>📡 Data: Yahoo Finance | Scan: κάθε βράδυ μετά κλείσιμο | Monitor: κάθε ώρα</div>
  </div>
...
def send_report():
    ...
    print(f"Sending report to {to_addr}...")
